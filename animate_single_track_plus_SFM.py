...
csv_file = "track_scene8_col1_from_1730818402995053312_to_1730818409995027200.csv"
...
def get_frame_rate(df):
    """
    Returns the frame rate (frames per second) based on the timestamp column in the DataFrame.
    Assumes timestamps are in nanoseconds.
    """
    if len(df) < 2:
        return None  # Not enough data to compute frame rate

    # Calculate time differences between consecutive frames (in nanoseconds)
    time_diffs = df['timestamp'].diff().dropna()
    median_diff_ns = time_diffs.median()

    # Convert nanoseconds to seconds for FPS calculation
...
def get_average_speed(df):
    """
    Calculates the average speed of the pedestrian in m/s from its positions in the CSV.
    Assumes timestamps are in nanoseconds and positions are in meters.
    """
    if len(df) < 2:
        return 0.0
    ...
    # Calculate total time in seconds (timestamps are in nanoseconds)
...
tolerance = 0.3  # stopping distance to goal
...
while np.linalg.norm(p.ra - p.g) > tolerance and t < t_max:
    ...
    # Store position and time for plotting
...
# After the simulation loop, animate the trajectories
positions = np.array(positions)
robot = np.array(robot)

fig, ax = plt.subplots(figsize=(8, 6))

# Plot the original positions from the CSV as background
...
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_title('Person and Robot Trajectories with Obstacle')
ax.legend()
ax.grid()
ax.axis('equal')
